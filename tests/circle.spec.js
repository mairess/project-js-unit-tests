...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(circle(17)).toMatchObject({ radius: 17, area: 907.46, circumference: 106.76 });
    // Teste se o objeto retornado possui 3 propriedades.
    const result = circle(3);
    const resultLength = Object.keys(result);
    expect(resultLength).toHaveLength(3)
    ...
    const results = circle(3);
    expect(results.area).toBeCloseTo(28.26);
